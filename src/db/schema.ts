...
export const toasts = sqliteTable('toasts', {
  ...
  toasterID: text('toasterID').notNull().references(() => users.userID), // FK to users (who sends the toast)
  toastieID: text('toastieID').notNull().references(() => users.userID), // FK to users (who receives the toast)
